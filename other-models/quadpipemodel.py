...
class AutoEncoder(nn.Module):
    ...
    def sample_latent(self,x):
        ...
        mean4=self.meanL4(x)
        sigma4=torch.sqrt(torch.exp(self.sigmaL4(x)))        
        self.mean=(mean1,mean2,mean3,mean4)
        self.sigma=(sigma1,sigma2,sigma3,sigma4)
        ...
        z4=mean4+sigma4*Variable(eps,requires_grad=False).cuda()
        z12=torch.cat((self.dec1(z1),self.dec2(z2)),dim=1)
        z34=torch.cat((self.dec3(z3),self.dec4(z4)),dim=1)
        z=torch.cat((z12,z34),dim=1)
        return z
